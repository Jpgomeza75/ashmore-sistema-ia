...
const stats = [
  { value: "200+ emails", label: "por deal activo al mes — coordinación con compradores, vendedores, abogados, auditores y reguladores" },
  { value: "3 deals", label: "simultáneos es el máximo que un equipo puede manejar sin que la calidad se deteriore" },
  { value: "0 visibilidad", label: "en tiempo real del estado de cada workstream — el socio se entera del problema cuando ya es tarde" },
];

const level1Outcomes = [
  { title: "Checklist dinámico de ejecución", desc: "Lista de tareas por fase del deal que se actualiza según el tipo de transacción" },
  { title: "Resumen de status automático", desc: "Update semanal generado a partir de emails y documentos del deal" },
  { title: "Preparación de data rooms", desc: "Organización y revisión preliminar de documentos para due diligence" },
  { title: "Alertas de deadlines y dependencias", desc: "Notificaciones cuando algo está por vencerse o bloqueado" },
];

const level2Outcomes = [
  { title: "Dashboard de deals activos", desc: "Estado de cada transacción en tiempo real con semáforos por workstream" },
  { title: "Automatización de workflows", desc: "Tareas recurrentes se ejecutan automáticamente según la fase del deal" },
  { title: "Coordinación multi-parte", desc: "Gestión centralizada de comunicaciones con todas las partes del deal" },
  { title: "Post-mortem de ejecución", desc: "Análisis automático de tiempos, cuellos de botella y lecciones por deal cerrado" },
];

const EjecucionGestionProcesoPage = () => {
      ...
      <main className="flex-1 flex flex-col min-h-0 px-4 md:px-6 lg:px-10 pt-4 pb-8 md:pb-16">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="shrink-0">
          <nav className="text-xs md:text-sm font-sans text-muted-foreground mb-2 flex items-center gap-1.5">
            ...
            <span>/</span>
            <span className="text-foreground font-medium">Ejecución y Gestión del Proceso</span>
          </nav>
          <div className="mb-3 md:mb-5">
            ...
            <h1 className="font-serif text-3xl md:text-5xl font-bold text-foreground leading-tight mb-2">Ejecución y Gestión del Proceso</h1>
            <p className="text-base md:text-xl font-sans text-muted-foreground">De gestionar deals por email y memoria a tener un sistema que no deja que nada se caiga.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-7 shrink-0 lg:auto-rows-fr">
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12, duration: 0.4 }}
            ...
            <p className="text-base md:text-lg font-sans text-muted-foreground leading-relaxed mb-4 md:mb-5">La IA se convierte en el project manager del deal: rastrea deadlines, prepara materiales y mantiene a todos informados.</p>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.4 }}
            ...
            <p className="text-base md:text-lg font-sans text-muted-foreground leading-relaxed mb-4 md:mb-5">Un sistema de gestión de deals que da visibilidad en tiempo real a todos los stakeholders y asegura que nada se caiga.</p>
            <div className="space-y-3 md:space-y-4 flex-1">
...
export default EjecucionGestionProcesoPage;
